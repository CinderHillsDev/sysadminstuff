...
// isBlockedHost() only recognizes IP literals — a hostname that merely *resolves*
// to an internal/reserved address sails right through it. Callers that connect to
// a user-supplied hostname (headers.js, tls.js) must resolve it first and check
// every answer here before connecting, or DNS rebinding bypasses the guard
// entirely. This is the one function in this file that does I/O.
export async function hostResolvesToBlocked(host) {
  ...
  const lookups = ['A', 'AAAA'].map(async (type) => {
    try {
      ...
      return (data.Answer || [])
        .filter((a) => (type === 'A' && a.type === 1) || (type === 'AAAA' && a.type === 28))
        .map((a) => a.data);
    } catch (e) {
      return [];
    }
  });
  const answers = (await Promise.all(lookups)).flat();
  return answers.some((ip) => isBlockedHost(ip));
}
